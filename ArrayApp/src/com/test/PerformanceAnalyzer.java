...
public class PerformanceAnalyzer {
   // static final Logger log = Logger.getLogger(PerformanceAnalyzer.class);
    ...
    static { 
        ...
        //isActive = Boolean.parseBoolean(AppSettings.getSettings().getPropValue("PAFlag"));
    }
    ...
    /**
     * 
     * @param pointMsg - to store original root point message/method name : .getClass().getEnclosingMethod().getName()
     */
    public static synchronized void start(String pointMsg) {
       ...
       System.out.println(sb.append(APP_LABEL).append(STARTING_LABEL).append(pointMsg!=null ?  pointMsg  : "").append(startDate).append(". Thread [{")
                .append(Thread.currentThread().getId()).append("} ").append(Thread.currentThread().getName())
                .append("]").append(NEW_LINE_STR).toString());
    ...
    public static synchronized void end() {
        ...
        if (instance != null && startDate != null) {
            ...
            System.out.println(sb.append(APP_LABEL).append(COMPLETING_LABEL).append(msg).append(endDate).append(". It took : ")
                    .append(min).append(" min. ").append(sec % 60).append(" sec.").append(". Thread [{")
                    .append(Thread.currentThread().getId()).append("} ").append(Thread.currentThread().getName())
                    .append("] ").append(NEW_LINE_STR).toString());
            sb.setLength(0);
            instance = null;
